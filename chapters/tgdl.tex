...
Il linguaggio è stato formulato come linguaggio  interpretato. 
In questa sezione vengono discusse delle possibili implementazioni 
delle parti maggiormente interessanti del linguaggio:
\begin{itemize}
    \item Gestore degli eventi
    \item Input injector
\end{itemize}
Non prese in considerazione le parti di tokenizzazione e parsing dei file, 
presupponendo la presenza di tutte le informazioni necessarie pronte all'utilizzo estratte
dall'albero di sintassi derivato dal passo precedenti di parsing.

\subsection{Processore di gioco}
Il game loop parte con l'avvio dell'entry point, tag setup:
\begin{enumerate}
    \item avvio del setup e passaggio del primo turno
    \item passaggio di turno 
	\item controllo al giocatore definito 
    \item avvio della funzione di fase
	\item sblocco delle azioni con PlayerChoice 
    \item controllo dei require
	\item fino ad altro passaggio di turno
\end{enumerate}

\subsection{Gestore degli eventi}
Il gestore degli eventi è il cuore dell'interprete si occupa di eseguire il codice 
del punto di entrata all'avvio, astratto tramite un evento speciale di avvio che 
da inizio al programma
