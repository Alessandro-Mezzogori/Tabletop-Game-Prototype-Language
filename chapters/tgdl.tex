...
\begin{verbatim}
[optional] [<modificatore>] input <tipo> <identificatore> [<filtri>]
\end{verbatim}
...
Esistono due modi principali per definirli:

\paragraph{Intra input}
Il metodo piú semplice per definire un tag é all'interno del blocco associato all'input.
\begin{verbatim}
...
\end{verbatim}
I filtri sono eseguiti su ogni oggetto tracciato di quel tipo, l'oggetto corrente é utilizzabile 
...
una funzione senza tipo di ritorno, inoltre é possibile definire solo un tag effetto all'interno 
...
disabilitata la chiamata non fallirá ma non avrà nessun effetto). \\
