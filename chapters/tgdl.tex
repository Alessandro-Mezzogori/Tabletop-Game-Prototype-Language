...
\subsubsection{Liste}
Le liste forniscono un insieme di oggetti accessibili tramite indici numerici interi, che mantiene l'ordine di inserimento. \\
Non hanno una dimensione fissa ma si adattano in base al numero di oggetti che contengono. \\
Per dichiarara una lista si utilizza il tipo lista:
Il tipo degli oggeti che si vuole raggruppare seguito da una coppia di parentesi quadre
\lstinline{<tipo>[]}. \\
L'inizializzazione é effettuata tramite un blocco in cui sono definibili un elenco di 
...
\begin{tabular}{| c | c | p{5cm} |}
    ...
    \hline    
    Rimuovi elemento ad indice & \lstinline|<lista>.removeAt(<indice>)|& rimuove l'oggetto all'indice corrispondente,
    sposta il resto della lista avanti di una posizione \\
    \hline    
...
Sono definite tramite il tag \lstinline|function <identificatore> <blocco funzione>|
...
Il tag di ritorno \lstinline|returns [<type>];| é un tag obbligatorio speciale non associato ad un blocco. \\
