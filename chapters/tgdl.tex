...
\\
Il blocco delimita l'ambito, o scope, e la vita di una variabile definita al suo interno ovvero
...
per indicare la coppia tag e il relativo blocco associato.
...
Tabletop Game Prototype Language si basa su un insieme di primitive da cui sono costruite tutte 
...
quando un reference object è copiato in una nuova variabile, nel nuovo oggetto si copierá
...
cifre, il tipo number equivale al tipo double presente in linguaggi come C, C\#, ecc...
...
I valori string sono immutabili (ovvero non modificabili), all'esecuzione di espressioni
che andrebbero a modificare il valore di una stringa, viene istanziata una nuova stringa con il valore
modificato.
...
Attualmente sono supportate tre tipologie: lista, table, stack, la cui sintassi è ispirata dalle
...
\lstinline|List| e una coppia di parentesi angolate con al loro interno il tipo degli 
...
In caso una espressione chiave venga valutata a null, il programma terminerà con errore.
...
\end{lstlisting}
Lo stack accetta valori nulli nelle funzioni di push ma non li aggiunge allo stack, trattandolo quindi
...
\begin{itemize}
    \item
    {
        ...
        L'espressione ha come tipo di ritorno un valore booleano, il cui valore corrisponde il risultato del controllo:
...
L'espressione di chiamata è utilizzata per chiamare l'esecuzione della funzione con lo 
...
si risolvono da sinistra verso destra. \\
...
L'istruzione di selezione è utilizzata per selezionare se eseguire o no il blocco d'istruzioni
...
L'istruzione for è un altro modo per creare cicli, utilizzata principalmente come abbreviazione
...
\begin{enumerate}
    ...
    \item {
        Espressione post ciclo: è un'espressione che viene valutata dopo l'esecuzione del blocco d'istruzioni 
...
se due o più giocatori si trovano a pari merito è sufficiente inserirli nella lista all'indice corrispondente 
...
Un attributo è una variabile associata alla definizione della classe in cui è dichiarato. 
Il valore in sé della variabile è legato all'oggetto ovvero all'istanza della classe. \\
Per definire un attributo si utilizza il tag \lstinline|attribute| come nella seguente sintassi:
\begin{lstlisting}
...
essere condivise tra molteplici giocatori, che possono accedere alle azioni fornite dalla classe come 
...
Il principale utilizzo di una classe di gruppo è creare dei gruppi di giocatori che abbiano
...
L'istanza è accessibile in qualsiasi blocco che accetti delle espressioni di accesso utilizzando 
...
\begin{itemize}
    ...
    \item Globale: il giocatore bersaglio è il giocatore attivo 
    \item {
        Gruppo, si hanno due casi:
        \begin{itemize}
            \item Se il giocatore attivo fa parte del gruppo sarà utilizzato come giocatore bersaglio
            \item Se il giocatore attivo non fa parte del gruppo, un giocatore a caso sarà utilizzato come giocatore bersaglio
        \end{itemize}
    }
\end{itemize}
...
filtrare tutte le istanze accettabili popolando la variabile definita dall'input. \\
...
\paragraph{Opzionale}
La parola chiave opzionale è un modificatore opzionale che permette alla variabile definita dall'input
...
\begin{itemize}
    \item Se l'input non è una lista allora il tipo del filtro sarà lo stesso dell'input 
    \item { 
...
\subsection{Funzioni degli eventi}
...
la risoluzione dell'albero degli eventi generato, al contrario un evento asincrono
...
Ogni mazzo possiede uno stack predefinito chiamato \lstinline|<stack>.discard_stack| che 
...
Un turno deve definire almeno una fase tramite il blocco \lstinline|phase|, che rappresenta 
...
\begin{tabularx}{\linewidth}{|l|p{6cm}|X|}
    ...
    \lstinline|)|
    &
    Passa il controllo al giocatore richiesto  e, se specificata, in una precisa fase \\
    \hline
...
che rappresentano una singola tessera all'interno di un gruppo del tabellone. \\
\begin{lstlisting}
...
\begin{figure}[ht]
\begin{center}
    \subfigure[Esempio coordinate con orientazione \textbf{row}]{\includegraphics{hex_row_num.png}}
    \subfigure[Esempio coordinate con orientazione \textbf{col}]{\includegraphics{hex_col_num.png}}
\end{center}    
\end{figure}
...
Le geometrie quadrate ed esagonali possiedono la tessera bianca che rappresenta lo spazio vuoto, 
...
L'ereditarietà consente di definire delle classi derivate che ereditano o eseguono un override degli attributi, funzioni e azioni 
...
\section{Molteplici file} \label{MultipleFiles}
...
Dentro il blocco del tag si possono definire delle qualsiasi istruzioni che saranno eseguite
prima di far partire il primo turno di gioco.

\newpage
\section{Esempio: Tris}
\begin{mdframed}[
...
struttura in CSharp, di seguito si possono trovare alcune delle conversioni 
meno triviali.
...
di acquisizione di esso dipendente dal modificatore definito.
...
\subsection{Utilizzo dell'assembly generato}
...
Il \textit{CodeProcessor} é il responsabile di eseguire effettivamente le istruzioni contenute nei blocchi tradotti, durante l'esecuzione 
